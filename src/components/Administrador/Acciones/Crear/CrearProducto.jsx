...
import { obtenerCategorias, obtenerMarcas, obtenerUnidades, crearProducto } from "@/lib/api";
...
export default function CrearProducto({ onCancel }) {
  ...
  const tipoPrincipalRef = useRef(null); // guarda si es 1 o 2
  ...
  const handleAgregarImagen = (event, tipo) => {
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      tipoPrincipalRef.current = tipo; // guarda tipo actual (1 o 2)
      reader.onloadend = () => {
  ...
  const handleImagenCortada = (blob) => {
    const tipo = tipoPrincipalRef.current; // recupera el tipo guardado
    ...
    const nuevoEstado = [...estadoFiltrado, nuevaImagen];
